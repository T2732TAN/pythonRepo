...
def relationship_status(from_member, to_member, social_graph):
    ...
    ftt = social_graph.get(from_member, {}).get("following", []).count(to_member)
    ttf = social_graph.get(to_member, {}).get("following", []).count(from_member)
    if ftt == ttf:
        if ftt >=1:
            return "friends"
        else:
...
def tic_tac_toe(board):
    ...
    def full(board):
        if checkH(board) != 0:
            return checkH(board)
        elif checkH(transpose(board)) != 0:
            return checkH(transpose(board))
        elif checkD(board) != 0:
            return checkD(board)
        else:
            return "NO WINNER"
